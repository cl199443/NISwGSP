...
void MultiImages::doFeatureMatching() const {
    ...
    for(int i = 0; i < images_data.size(); ++i) {
        const vector<Point2> & vertices = images_data[i].mesh_2d->getVertices();   //  vertices.emplace_back(w * lw, h * lh);  绘制网格顶点坐标
	...
	//////////////////////////////////////////////////
	//cout << "    multiImages1" << endl;
    const vector<vector<vector<Point2> > > & feature_matches = getFeatureMatches();  // 特征点配准，删除误匹配，计算单应矩阵
	//cout << "    multiImages2" << endl;
    ...
    for(int i = 0; i < images_match_graph_pair_list.size(); ++i) 
	{
        ...
        APAP_Stitching::apap_project(feature_matches[m1][m2],
                                     ...
                                     images_data[m1].mesh_2d->getVertices(), apap_matching_points[m1][m2], apap_homographies[m1][m2]);   //Moving-DLT
        APAP_Stitching::apap_project(feature_matches[m2][m1],
                                     feature_matches[m1][m2],
                                     images_data[m2].mesh_2d->getVertices(), apap_matching_points[m2][m1], apap_homographies[m2][m1]); //Moving-DLT
		...
		/*
					DMatch : 
					CV_PROP_RW int queryIdx;	   // query descriptor index  要匹配的特征描述子索引
					CV_PROP_RW int trainIdx;      // train descriptor index 被匹配的特征描述子索引
					CV_PROP_RW int imgIdx;       // train image index  被匹配图像的索引
					CV_PROP_RW float distance;  //二个特征描述子之间的欧氏距离
		*/
...
const vector<detail::MatchesInfo> & MultiImages::getPairwiseMatchesByMatchingPoints() const {
    if(pairwise_matches.empty()) {
		...
		//cout << "   multi-getpair1" << endl;
        doFeatureMatching();
		//cout << "   multi-getpair2" << endl;
		///////////////////
    }
    ...
    if(camera_params.empty()) {
        ...
        for(int i = 0; i < camera_params.size(); ++i) {
            if(image_focal_candidates[i].empty()) {
                ...
                Statistics::getMedianWithoutCopyData(image_focal_candidates[i], camera_params[i].focal);  //取所有网格估计的焦距中值作为初始focal
            }
        }
        /********************/
        /*** 3D Rotations ***/
...
const vector<vector<InterpolateVertex> > & MultiImages::getInterpolateVerticesOfMatchingPoints() const {  //获取特征点_p所在的网格编号，并计算与网格四个顶点的线性插值权值
    if(mesh_interpolate_vertex_of_matching_pts.empty()) { 
        mesh_interpolate_vertex_of_matching_pts.resize(images_data.size());
        const vector<detail::ImageFeatures> & images_features = getImagesFeaturesByMatchingPoints();   //判断特征点是否匹配完成，未匹配则进行特征点配准，否则进行特征点所在网格的定位操作
        for(int i = 0; i < mesh_interpolate_vertex_of_matching_pts.size(); ++i) {
            mesh_interpolate_vertex_of_matching_pts[i].reserve(images_features[i].keypoints.size()); //分配存储空间
            for(int j = 0; j < images_features[i].keypoints.size(); ++j) {
                mesh_interpolate_vertex_of_matching_pts[i].emplace_back(images_data[i].mesh_2d->getInterpolateVertex(images_features[i].keypoints[j].pt)); //返回所有特征点的网格编号和插值权重
            }
        }
    }
...
const vector<SimilarityElements> & MultiImages::getImagesSimilarityElements(const enum GLOBAL_ROTATION_METHODS & _global_rotation_method) const {
    ...
    if(result.empty()) {
        result.reserve(images_data.size());  //including scale && theta
        const vector<detail::CameraParams> & camera_params = getCameraParams(); //focal,aspect,ppx,ppy,T,R
        for(int i = 0; i < images_data.size(); ++i) {
            result.emplace_back(fabs(camera_params[parameter.center_image_index].focal / camera_params[i].focal),   //尺度因子为 S-i = f-center/f-i
...
const vector<vector<double> > & MultiImages::getImagesGridSpaceMatchingPointsWeight(const double _global_weight_gamma) const {
    if(_global_weight_gamma && images_polygon_space_matching_pts_weight.empty()) 
	{
		...
		{ //i stands for the image 
            const int polygons_count = (int)images_data[i].mesh_2d->getPolygonsIndices().size();  //图像i的网格数目（nw*nh）
            ...
            for(int j = 0; j < images_features_mask[i].size(); ++j) {
                if(images_features_mask[i][j]) {
                    polygons_has_matching_pts[mesh_interpolate_vertex_of_matching_pts[i][j].polygon  /* 图像i的第j个特征点所在的网格编号*/ ] = true;  //该网格内有特征点匹配对
                }
            }
            ...
            for(int j = 0; j < polygons_has_matching_pts.size(); ++j) {
                if(polygons_has_matching_pts[j]) {
                    ...
                    que.push(dijkstraNode(j, j, 0));     //_from,_pos,_dis
                } else {
                    images_polygon_space_matching_pts_weight[i].emplace_back(MAXFLOAT);
                }
            }
            const vector<Indices> & polygons_neighbors = images_data[i].mesh_2d->getPolygonsNeighbors();//网格的相邻网格编号
            const vector<Point2> & polygons_center = images_data[i].mesh_2d->getPolygonsCenter(); //网格的center坐标
            while(que.empty() == false) 
			{
                const dijkstraNode now = que.top();  //the last pushed  element 
                ...
                if(polygons_has_matching_pts[index] == false) 
				{
					...
					{
                        const int n = polygons_neighbors[index].indices[j];  //邻居网格编号
...
const vector<vector<vector<pair<int, int> > > > & MultiImages::getFeaturePairs() const {
    if(feature_pairs.empty()) {
		...
		{		
			...
			////////////////////////////////////////////
		//	cout << " mul-getInitialFeaturePairs1" << endl;
            const vector<pair<int, int> > & initial_indices = getInitialFeaturePairs(match_pair);  //配对信息
			//////////////////////////////////////////
			//cout << "  mul-getInitialFeaturePairs2" << endl;
            ...
            for(int j = 0; j < initial_indices.size(); ++j) 
			{
                ...
                Y.emplace_back(m2_fpts[it.second]); //存储配对的特征点坐标信息
            }
            vector<pair<int, int> > & result = feature_pairs[match_pair.first][match_pair.second];
            result = getFeaturePairsBySequentialRANSAC(match_pair, X, Y, initial_indices);         //RANSAC算法，剔除特征点误匹配
            assert(result.empty() == false);
#ifndef NDEBUG
            writeImageOfFeaturePairs("sRANSAC", match_pair, result);  //打印输出特征点误匹配删除后的匹配结果信息
#endif
        }
...
const vector<vector<vector<Point2> > > & MultiImages::getFeatureMatches() const {
    if(feature_matches.empty()) {
		//////////////////////////////////////
		//cout << " multi-getfeaturepaires1" << endl;

        const vector<vector<vector<pair<int, int> > > > & feature_pairs = getFeaturePairs();  //获取特征点对匹配信息

		//cout << " multi-getfeaturepaires2" << endl;
     ...
     return feature_matches;
																											 ...
																											 //使用RANSAC算法剔除特征点误匹配信息，提高特征配准率
vector<pair<int, int> > MultiImages::getFeaturePairsBySequentialRANSAC(const pair<int, int> & _match_pair,  //有重叠区域的二幅图像
                                                                       const vector<Point2> & _X,		   //粗匹配特征点对的X坐标
                                                                       const vector<Point2> & _Y,		  //粗匹配特征点对的Y坐标
                                                                       const vector<pair<int, int> > & _initial_indices) const {   //粗匹配特征点对信息
...
#ifndef NDEBUG
    cout << "Global true Probabiltiy = " << result.size() / (float)_initial_indices.size() << endl;  //XX比率
#endif
    return result;
}
...
vector<pair<int, int> > MultiImages::getInitialFeaturePairs(const pair<int, int> & _match_pair) const {   //特征点的预设粗匹配: 基于特征描述子的欧氏距离，进行最小距离粗匹配
    const int nearest_size = 2, pair_count = 1;
    const bool ratio_test = true, intersect = true;
    
    assert(nearest_size > 0);
    
    const int feature_size_1 = (int)images_data[_match_pair.first ].getFeaturePoints().size();				//待拼接匹配相邻图像的第一幅图像的特征点坐标信息
    const int feature_size_2 = (int)images_data[_match_pair.second].getFeaturePoints().size();			   //待拼接匹配相邻图像的第二幅图像的特征点坐标信息
    ...
    for(int p = 0; p < pair_count; ++p)
	{
		...
		//	cout << "it->distance is " << it->distance << "  " << feature_distance_set.size() << endl;
			if (ratio_test)
			{
				if (feature_distance_set.size()>1)
				{
					const set<FeatureDistance>::const_iterator it2 = std::next(it, 1);

					if (nearest_k == nearest_size && it2->distance * FEATURE_RATIO_TEST_THRESHOLD > it->distance)
					{
						continue;
					}
...
#ifndef NDEBUG
    writeImageOfFeaturePairs("init", _match_pair, initial_indices);   //打印输出特征点粗匹配信息
#endif
    return initial_indices;
}

Mat MultiImages::textureMapping(const vector<vector<Point2> > & _vertices,		  //网格变换后的顶点坐标集合
                                const Size2 & _target_size,					     //映射蒙版大小
                                const BLENDING_METHODS & _blend_method) const { //融合方法
...
Mat MultiImages::textureMapping(const vector<vector<Point2> > & _vertices,
    ...
    vector<Rect_<FLOAT_TYPE> > rects = getVerticesRects<FLOAT_TYPE>(_vertices);   //获取每幅图像网格变换后的左上角网格坐标以及对应映射区域的蒙版尺寸
...
// 输出相邻二幅图之间的特征点匹配信息: eg :  IMAGE0--IMAGE1  init feature pairs = 525，并将匹配结果信息写入相关文件
void MultiImages::writeImageOfFeaturePairs(const string & _name,
    ...
    imwrite(parameter.debug_dir +
            ...
            images_data[_match_pair.first ].file_extension, image_of_feauture_pairs);		 //将打印特征点匹配信息的图像结果写入文件路径
}